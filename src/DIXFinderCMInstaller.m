...
- (NTFileDesc*) installedExtensionDesc
{
	if ( _installedExtension == nil )
	{
		...
		if ( ![extensionDesc isDIXFinderCM] )
		{
			...
			extensionPath = [self extensionPathForDomain: kSystemDomain];
...
- (BOOL) installToDomain: (int) domain
{
	...
	if ( ![[NSWorkspace sharedWorkspace] performFileOperation: NSWorkspaceCopyOperation
													   source: [builtInExtensionDesc parentPath: NO /*forDisplay*/]
												  destination: extensionPath
														files: [NSArray arrayWithObject: [builtInExtensionDesc name]]
														  tag: &tag] )
/*
...
- (BOOL) remove
{
	...
	if ( installedExtensionDesc != nil )
	{
		...
		if ( ![[NSWorkspace sharedWorkspace] performFileOperation: NSWorkspaceDestroyOperation
														   source: [installedExtensionDesc parentPath: NO /*forDisplay*/]
													  destination: @""
															files: filesToTrash
															  tag: &tag] )
		/*
