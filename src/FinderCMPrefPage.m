...
- (void) awakeFromNib
{
	...
	NSButtonCell *userDomainRadioButton = (NSButtonCell*)[[_domainMatrix cells] objectAtIndex: 1];
...
- (IBAction) install: (id) sender
{
	...
	NSButtonCell *localDomainRadioButton = (NSButtonCell*)[[_domainMatrix cells] objectAtIndex: 0];
...
- (IBAction) revealInFinder: (id) sender
{
	...
	NTFileDesc *installedExtension = [installer installedExtensionDesc];
	if ( installedExtension != nil )
        [[NSWorkspace sharedWorkspace] selectFile: [installedExtension path] inFileViewerRootedAtPath: @""];
}
